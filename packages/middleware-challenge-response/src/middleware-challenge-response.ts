...
export class MiddlewareChallengeResponse implements Startable, MiddlewareChallengeResponse {
  ...
  handle ({ stream, connection }: { stream: Stream, connection: Connection }): void {
    this.log('📥 RECEIVED AUTH CONNECTION REQUEST', connection.id, connection.remotePeer.toString())
  ...
  async wrap (connectionId: string, abortOptions?: AbortOptions): Promise<boolean> {
    ...
    if (!this.started) {
      this.log('❌ challenge-response middleware not started')
      return false
    }
    ...
    this.log(`Initiating authentication for connection ${connectionId}`)
    ...
    try {
      ...
      this.log('❌❌❌ Authentication error for connection', connectionId, err)
      this.log(`Authentication error for connection ${connectionId.slice(0, 8)}: ${err.message}`)
      return false
    }
